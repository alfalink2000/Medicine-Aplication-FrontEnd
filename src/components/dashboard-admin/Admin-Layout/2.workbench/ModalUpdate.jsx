...
import { UpdateExtranjero } from "../../../../actions/extranjeros";
...
export const ModalUpdate = () => {
  ...
  useEffect(() => {
    const selectedUser =
      extranjeros.find((user) => user.id === extranjeroCharged) || {};
    setValues(selectedUser);
  }, [extranjeroCharged, extranjeros]);
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      dispatch(UpdateExtranjero(values));
      setValues(initialState); // Reset the form after successful submission
      closeModal();
    }
  };
  ...
  const openModal = () => {
    if (!extranjeroCharged) {
      // Muestra un SweetAlert2 si extranjeroCharged no está disponible
      Swal.fire({
        icon: "warning",
  ...
  return (
    <div>
      <button onClick={openModal}>
        <img
          src={process.env.PUBLIC_URL + "/assets/mod-user.png"}
          alt="Modificar"
        />
      ...
      {extranjeroCharged && (
        ...
        >
          ...
          <form className="form-register">
            <input type="hidden" name="id" value={extranjeroCharged} />
            <input
              type="text"
              autoComplete="off"
              name="nombre"
